...
@register_adapter("cdc_fluview")
class CDCFluViewAdapter(IndicatorSourceAdapter):
    source_code = "cdc_fluview"
    ...
    def fetch(
        ...
        if not regions or not start_week or not end_week:
            logger.warning("CDCFluViewAdapter missing regions or epiweek window; returning empty.")
            return []
        ...
        if data.get("result") != 1:
            logger.warning("CDC FluView returned result=%s", data.get("result"))
            return []
